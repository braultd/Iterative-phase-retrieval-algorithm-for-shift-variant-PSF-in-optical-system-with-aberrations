function [Uprop] = SVPSF_Propagation(ConvolutionKernel,Weights,mtfON,x)
% Author : Dylan Brault
...
for k=1:size(ConvolutionKernel,3)
    ...
    % Computation of the Fourier transform of k-th mode
    FTConvKern=fft2(ConvolutionKernel(:,:,k));
    % Setting the phase at the origin of FTConvKern (thus, the phase origin
    % is on the hologram plane.
    FTConvKern(1,1)=abs(FTConvKern(1,1));

    % Computation of the shift-variant convolution
    Uprop=Uprop+ifft2(fft2(Weights(:,:,k).*x).*FTConvKern.*mtfON);
end
